...
def _get_vgname(devNodePath):
    ...
    return re.findall(r"LVM2_VG_NAME='([^\']*)'", out.decode('utf-8'))[0]
...
def _is_available(name, devtype, fstype, mountpoint, majmin, devs=None):
    ...
    if (devtype in ['part', 'disk', 'mpath'] and
            fstype in ['', 'LVM2_member']
            and mountpoint == ''
            and not has_VG
            and _is_dev_leaf(devNodePath, name, devs, devtype)
            and not _is_dev_extended_partition(devtype, devNodePath)):
        return True
    return False
...
def lvs(vgname=None):
    ...
    # provided
    lvs = list(filter(lambda f: vgname is None or vgname in f,
...
def pvs(vgname=None):
    ...
    # provided
    pvs = list(filter(lambda f: vgname is None or vgname in f,
                      map(lambda v: v.strip(), out.strip('\n').split('\n'))))
